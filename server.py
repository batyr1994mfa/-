...
# Скользящее окно контекста: сколько последних сообщений истории чата
# реально уходит в модель на каждый запрос (полная история всё равно хранится
# на диске и видна в интерфейсе). ~20 сообщений — это примерно 8-10 полных
# обменов реплика/ответ с запасом на tool-цепочки, разумный баланс между
# связностью диалога и скоростью/расходом токенов на длинных чатах.
...
class State:
    def __init__(self):
        ...
        # Предпочитаемые сайты по категориям поиска — если список пуст, ИИ
        # сам выбирает, где искать (никаких ограничений). Если непуст —
        # модель получает инструкцию искать в первую очередь именно там
        # (через site:домен в запросах google_search/web_search, либо через
        # прямой fetch_url по этим доменам).
        self.preferred_sites_knowledge = []
        self.preferred_sites_shopping = []
        self.preferred_sites_photo = []
        self.preferred_sites_video = []
        # Категории инструментов, вручную выключенные пользователем через
        # панель "🔧" у поля ввода — их схемы вообще не попадают в промпт.
        # По умолчанию всё включено (пустой список = ничего не выключено).
        self.disabled_tool_categories = []
        # Режим панели "Pro": "auto" — сервер сам определяет нужные категории
        # инструментов по ключевым словам в сообщении (см. auto_detect_categories);
        # "manual" — используется disabled_tool_categories как задал пользователь.
        self.tools_mode = "auto"
    ...
    def recent_messages_for(self, chat_id: str, limit: int = HISTORY_WINDOW_MESSAGES) -> list:
        """То же самое, что messages_for(), но обрезает историю до последних
        N сообщений перед отправкой в Ollama — 'скользящее окно контекста'.
        Полная история по-прежнему хранится на диске и видна в интерфейсе,
        обрезается только то, что реально уходит в модель на КАЖДЫЙ запрос:
        без этого каждое новое сообщение в долгом чате пересылало бы всю
        переписку целиком, раздувая промпт и линейно замедляя генерацию по
        мере роста диалога. tool-сообщения не считаем отдельно от их пары
        (assistant с tool_calls + result) — обрезаем по границе user-хода,
        чтобы не разорвать вызов инструмента посреди результата."""
        full = self.messages_for(chat_id)
        if len(full) <= limit:
            return full
        # Ищем ближайшую точку среза НЕ посреди tool-цепочки: пятимся назад
        # от límite, пока не наткнёмся на сообщение с role == "user" — начинаем
        # окно с него, чтобы не оставить "осиротевший" tool-результат без
        # предшествующего вызова.
        start = len(full) - limit
        while start > 0 and full[start].get("role") != "user":
            start -= 1
        return full[start:]
    ...
    def load(self):
        ...
        for key in ("preferred_sites_knowledge", "preferred_sites_shopping",
                    "preferred_sites_photo", "preferred_sites_video"):
            val = data.get(key)
            if isinstance(val, list):
                setattr(self, key, [str(x).strip() for x in val if str(x).strip()])
        dtc = data.get("disabled_tool_categories")
        if isinstance(dtc, list):
            self.disabled_tool_categories = [c for c in dtc if c in TOOL_CATEGORIES]
        if data.get("tools_mode") in ("auto", "manual"):
            self.tools_mode = data.get("tools_mode")
    ...
    def save(self):
        try:
            ...
            data = {
                ...
                "preferred_sites_knowledge": self.preferred_sites_knowledge,
                "preferred_sites_shopping": self.preferred_sites_shopping,
                "preferred_sites_photo": self.preferred_sites_photo,
                "preferred_sites_video": self.preferred_sites_video,
...
def tool_read_file(path: str) -> str:
    ...
    if target.suffix.lower() in _BINARY_SKIP_EXTENSIONS or _looks_binary(target):
        return (
            f"Ошибка: '{path}' похож на бинарный файл — читать его как обычный текст "
            f"бессмысленно (получится мешанина байтов). Используй подходящий инструмент: "
            f"read_document для PDF/DOCX/XLSX, archive_extract для архивов, view_image/"
            f"ocr_image для изображений, или просто list_dir, если нужно только узнать, "
            f"что за файл и какого он размера."
        )
    try:
...
_BINARY_SKIP_EXTENSIONS = {
    ".zip", ".tar", ".gz", ".bz2", ".xz", ".7z", ".rar", ".exe", ".dll", ".so", ".bin",
    ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx", ".odt", ".ods",
    ".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp", ".ico", ".svg", ".tiff",
    ".mp3", ".mp4", ".avi", ".mkv", ".mov", ".wav", ".flac", ".ogg", ".webm",
    ".db", ".sqlite", ".sqlite3", ".pyc", ".pyo", ".class", ".o", ".a", ".ttf", ".otf",
    ".woff", ".woff2", ".appimage", ".deb", ".rpm", ".iso", ".img", ".dat",
}


def _looks_binary(fpath: Path) -> bool:
    """Быстрая эвристика: пытаемся прочитать первые байты файла и смотрим,
    похоже ли на бинарные данные (нулевой байт — почти 100% гарантия, что
    это не текст). Нужна как подстраховка для файлов с незнакомым/отсутствующим
    расширением, которых нет в _BINARY_SKIP_EXTENSIONS."""
    try:
        with open(fpath, "rb") as f:
            chunk = f.read(2048)
        return b"\x00" in chunk
    except Exception:
        return True  # не смогли прочитать — на всякий случай пропускаем
    ...
    for root_dir in roots:
        ...
        for root, _dirs, files in os.walk(root_dir):
            for fname in files:
                ...
                # Бинарные файлы (архивы, картинки, исполняемые файлы и т.п.)
                # пропускаем целиком: чтение их как текста даёт мешанину из
                # случайных байтов, которая не только бесполезна как результат
                # поиска, но и "отравляет" дальнейший контекст диалога, если
                # модель попытается её процитировать/описать.
                if fpath.suffix.lower() in _BINARY_SKIP_EXTENSIONS:
                    continue
                try:
...
def tool_recall_solutions(query: str, max_results: int = 5) -> str:
    ...
    scored = [x for x in scored if x[1] >= 2]  # минимум 2 общих слова — иначе ловит случайные совпадения
...
def solutions_auto_digest(user_text: str, max_items: int = 2) -> str:
    ...
    if not items:
        return ""
    # Порог совпадения по словам — намеренно НЕ снижаем его даже когда архив
    # стоит первым в приоритете: снижение до 1 слова на практике оказалось
    # слишком агрессивным — архив начинал подмешиваться в вообще не связанные
    # вопросы (например, единственная сохранённая задача про LM Studio
    # неизбежно "совпадала" по случайному слову почти с любым техническим
    # вопросом). Обязательный вызов recall_solutions (см.
    # knowledge_priority_instruction) обеспечивает проверку архива и без
    # ослабления порога — а найденное модель обязана сверять на релевантность
    # сама, а не слепо использовать любой результат.
    ...
    if not scored:
        if archive_is_top:
            ...
            # (без решений) как явный намёк модели, что архив непустой.
            titles = [it["task"] for it in items[:8]]
            return (
                "Архив успешных решений НЕ пуст, но ни одна запись не совпала по ключевым "
                "словам с текущим запросом — заголовки сохранённых задач: " + "; ".join(titles) +
                ". Раз архив в приоритете (1-е место), можешь вызвать recall_solutions с сутью "
                "текущего запроса, ЕСЛИ вопрос пользователя действительно похож на одну из этих "
                "задач. Если же вопрос явно про другое (например, просто общий вопрос без связи "
                "с перечисленными задачами) — НЕ вызывай recall_solutions и не упоминай архив "
                "вообще, просто ответь на заданный вопрос по существу."
            )
        return ""
    scored.sort(key=lambda x: x[1], reverse=True)
...
_CATEGORY_KEYWORDS = {
    "web": ("найди", "поищи", "поиск", "интернет", "гугл", "google", "яндекс", "погода",
            "курс валют", "ссылк", "сайт", "видео", "фото", "картинк", "youtube", "wikipedia",
            "скачай", "download", "search"),
    "files": ("файл", "папк", "загрузк", "прочитай", "рабочем столе", "документ",
              "директор", "ярлык", "скрипт", "code", "директория"),
    "system": ("терминал", "запусти", "bash", "команда", "установи", "процесс",
               "systemctl", "git ", "сервис", "службу"),
    "memory": ("запомни", "вспомни", "архив", "решени", "заметк"),
    "misc": ("погода", "курс валют", "напомни", "telegram", "rss", "письмо"),
}


def auto_detect_categories(user_text: str) -> set:
    """Режим АВТО панели 'Pro': по ключевым словам в сообщении определяет,
    какие категории инструментов потенциально понадобятся, вместо ручных
    тумблеров. Консервативно — лучше лишний раз включить категорию, чем
    сломать реальную задачу пользователя."""
...
TOOLS_SCHEMA = [
    ...
    {"type": "function", "function": {
        "name": "search_files",
        "description": (
            "Найти ТЕКСТ ВНУТРИ содержимого файлов (рекурсивно по подключённым папкам или "
            "указанной подпапке) — например, найти, где упоминается конкретная строка/функция. "
            "Автоматически пропускает бинарные файлы (архивы, картинки и т.п.). "
            "НЕ используй этот инструмент, если просто нужно посмотреть, какие файлы/папки "
            "лежат внутри директории — для этого используй list_dir, он быстрее и не читает "
            "содержимое файлов."
        ),
    ...
    {"type": "function", "function": {
        ...
        "description": (
            ...
            "инструмент, если хочешь поискать целенаправленно по другому запросу. Если "
            "результат явно не отвечает на текущий вопрос пользователя — игнорируй его, "
            "не притягивай нерелевантное решение к ответу."
        ),
        "parameters": {"type": "object", "properties": {
            "query": {"type": "string"},
...
# Эти три инструмента добавляются в схему, только если соответствующий канал
# реально настроен (ntfy_topic / SMTP / Telegram-бот) — иначе они бы просто
# занимали место в промпте на каждый запрос без единого шанса быть полезными.
TOOL_SCHEMA_NOTIFY = {"type": "function", "function": {
    "name": "notify",
    "description": "Отправить push-уведомление пользователю на телефон/десктоп через ntfy.sh.",
    "parameters": {"type": "object", "properties": {
        "message": {"type": "string"}, "title": {"type": "string"},
    }, "required": ["message"]},
}}
TOOL_SCHEMA_EMAIL = {"type": "function", "function": {
    "name": "send_email",
...
TOOL_SCHEMA_TELEGRAM = {"type": "function", "function": {
    "name": "telegram_send",
...
def get_tools_schema(user_text: str = "") -> list:
    ...
    notify/send_email/telegram_send добавляются, только если соответствующий канал
    реально настроен.

    Категории инструментов управляются панелью "Pro":
    - tools_mode == "manual": берём state.disabled_tool_categories как задал пользователь.
    - tools_mode == "auto" (по умолчанию): сервер сам определяет нужные категории по
      ключевым словам в тексте сообщения (auto_detect_categories) — всё остальное
      исключается из схемы, экономя токены без ручной возни с тумблерами."""
    if state.tools_mode == "manual":
        disabled_keys = set(state.disabled_tool_categories)
    else:
        needed = auto_detect_categories(user_text)
        disabled_keys = set(TOOL_CATEGORIES.keys()) - needed

    disabled_names = set()
    for cat_key in disabled_keys:
        _, names = TOOL_CATEGORIES.get(cat_key, (None, set()))
        disabled_names |= names

    schema = [t for t in TOOLS_SCHEMA if t["function"]["name"] not in disabled_names]
    if state.ai_autonomy:
        schema += [TOOL_SCHEMA_SHELL, TOOL_SCHEMA_SYSTEMD, TOOL_SCHEMA_PYREPL]
    if state.ntfy_topic:
        schema.append(TOOL_SCHEMA_NOTIFY)
    if state.smtp_host and state.smtp_user and state.smtp_password:
        schema.append(TOOL_SCHEMA_EMAIL)
    if state.telegram_bot_token and state.telegram_chat_id and "misc" not in disabled_keys:
        schema.append(TOOL_SCHEMA_TELEGRAM)
    return schema
...
def knowledge_priority_instruction() -> str:
    ...
    if order and order[0] == "archive":
        lines.append(
            "Архив стоит на 1-м месте — это значит, что перед тем как решать КОНКРЕТНУЮ "
            "техническую задачу (запуск программы, скрипт, настройка системы), которая уже "
            "могла решаться раньше, ты ОБЯЗАН явно вызвать recall_solutions с сутью текущего "
            "запроса, даже если тебе кажется, что там ничего нет — не полагайся только на "
            "автоматическое подмешивание, оно может не сработать при другой формулировке. "
            "ВАЖНО: это правило НЕ относится к обычным вопросам не про повтор задачи (общие "
            "вопросы, вопросы про твоё поведение/настройки, светская беседа) — для них архив "
            "не проверяем вообще. И даже если ты вызвал recall_solutions: если то, что он "
            "вернул, явно не отвечает на заданный пользователем вопрос — просто проигнорируй "
            "результат и ответь на реальный вопрос по существу, не пытайся притянуть "
            "нерелевантное решение за уши."
        )
    return "\n".join(lines)


def preferred_sites_instruction() -> str:
    categories = [
        ("knowledge", state.preferred_sites_knowledge,
         "поиска знаний/решений технических вопросов (google_search, web_search, fetch_url)"),
        ("shopping", state.preferred_sites_shopping,
         "поиска товаров/покупок (google_search, web_search, fetch_url)"),
        ("photo", state.preferred_sites_photo,
         "поиска фото (в дополнение к image_search)"),
        ("video", state.preferred_sites_video,
         "поиска видео (в дополнение к video_search)"),
    ]
    active = [(key, sites, desc) for key, sites, desc in categories if sites]

    url_rule = (
        "Если сам пользователь в сообщении прислал конкретную ссылку (например 'зайди на "
        "https://... и найди там...') — это ВСЕГДА приоритетнее любых настроенных "
        "предпочитаемых сайтов и любых других инструментов: используй fetch_url именно на "
        "эту ссылку в первую очередь, не подменяй её google_search или другим сайтом по "
        "своей инициативе."
    )
    if not active:
        return url_rule

    lines = ["Пользователь настроил предпочитаемые сайты для отдельных категорий поиска:"]
    for key, sites, desc in categories:
        if sites:
            lines.append(f"— Для {desc}: {', '.join(sites)}.")
        else:
            lines.append(f"— Для {desc}: не заданы, выбирай сам, где искать.")
    lines.append(
        "Для категорий с заданными сайтами: в google_search/web_search формируй запрос с "
        "ограничением по сайту, например 'site:ozon.ru OR site:wildberries.ru <запрос>' — "
        "перечисли через OR ВСЕ сайты категории, и НЕ обращайся к другим сайтам за пределами "
        "этого списка для данной категории, даже если кажется, что там будет лучше. Если "
        "после этого нужно открыть конкретную страницу — используй fetch_url на найденной "
        "ссылке (тоже с одного из заданных сайтов). Для фото/видео с заданными сайтами "
        "сначала попробуй найти именно там через google_search с 'site:' и открыть через "
        "fetch_url, и только если совсем не получилось — используй image_search/video_search "
        "как запасной вариант. Категории с пустым списком сайтов ничем не ограничены — "
        "используй любые источники и обычные инструменты как всегда."
    )
    lines.append(url_rule)
    return "\n".join(lines)


# ------------------------- Турбо-режим (Fast Path) --------------------------
#
# Проблема: раньше КАЖДОЕ сообщение — даже "привет" в абсолютно пустом новом
# чате — получало полный набор из ~30 схем инструментов (get_tools_schema)
# и тяжёлый многоабзацный системный промпт (инструкции про архив, приоритет
# знаний, предпочитаемые сайты, фото/видео и т.д.). Сама схема инструментов
# в сериализованном виде — это уже несколько тысяч токенов ДО единого слова
# реального диалога, отсюда и медленные ответы, и полный сброс кэша
# контекста Ollama на голом приветствии. Турбо-режим обходит всё это для
# коротких сообщений без явного намерения на действие.

_FAST_PATH_MAX_LEN = 20
_FAST_PATH_ACTION_WORDS = (
    "покажи", "найди", "скачай", "запусти", "открой", "создай", "сделай",
    "удали", "измени", "отредактируй", "прочитай", "напиши", "проверь",
    "установи", "запомни", "вспомни", "сохрани", "погода", "курс",
    "переведи", "посчитай", "search", "find", "show", "run", "open",
    "create", "download", "delete", "write", "read", "install",
)


def is_fast_path_message(user_text: str, has_files: bool) -> bool:
    """True для коротких реплик без признаков задачи (приветствие, проверка
    связи вроде 'ты тут?') — для них не нужны ни инструменты, ни RAG-контекст
    папок, ни размышления. Любой намёк на действие (ключевые слова, файлы,
    длина текста) выводит из турбо-режима в обычный полный путь."""
    if has_files:
        return False
    ...
    if not text or len(text) > _FAST_PATH_MAX_LEN:
        return False
    low = text.lower()
    if any(w in low for w in _FAST_PATH_ACTION_WORDS):
        return False
    if "?" in text and len(text) > 12:
        # короткий, но содержательный вопрос (не просто "ты тут?") — на всякий
        # случай не считаем однозначно "болтовнёй", пусть обрабатывается полным путём
        return False
    return True


def build_minimal_system_message() -> dict:
    """Облегчённый системный промпт для турбо-режима: без списка инструментов,
    без перечисления подключённых папок, без длинных инструкций про архив/
    приоритеты источников — оставляем только персону модели (если задана) и
    короткую заметку про язык ответа."""
    custom_system = get_model_custom_system(state.model)
    parts = [custom_system] if custom_system else []
    parts.append("Отвечай кратко и по-русски (если персона выше не указывает иное).")
...
def build_system_message(user_text: str = "") -> dict:
    ...
    sites_instruction = preferred_sites_instruction()
    if sites_instruction:
        parts.append(sites_instruction)
    ...
    parts.append(
        ...
        "git_tool (git-команды), telegram_send (требует настроенного Telegram-бота в "
        "боковой панели), lint_format (проверка/форматирование кода), get_weather, "
        "get_exchange_rate, rss_read и set_reminder (напоминание через N минут)."
    )
...
def stream_ollama(messages, cancel_flag=None, fast_path=False, user_text=""):
    base_payload = {"model": state.model, "messages": messages, "stream": True,
                     "think": False if fast_path else state.think_enabled,
                     "options": state.reasoning_options()}

    # Турбо-режим: вообще не отправляем схему инструментов — экономит тысячи
    # токенов и позволяет модели ответить за секунды на простую реплику,
    # вместо того чтобы "продираться" через полный список из ~30 функций.
    payload = base_payload if fast_path else dict(base_payload, tools=get_tools_schema(user_text))
...
def api_chat():
    ...
    def generate():
        ...
        def worker():
            fast_path = is_fast_path_message(user_text, bool(files))
            if fast_path:
                # Турбо-режим: без RAG подключённых папок, без схемы
                # инструментов, без размышлений — минимальный промпт для
                # мгновенного ответа на "привет"/"ты тут?" и подобное.
                messages = [build_minimal_system_message()] + state.recent_messages_for(target_chat_id)
            else:
                messages = [build_system_message(user_text)] + state.recent_messages_for(target_chat_id)
                # Дайджест архива решений подмешиваем в КОПИЮ последнего
                # (текущего) сообщения пользователя, а не в системный промпт:
                # содержимое системного промпта иначе менялось бы от хода к
                # ходу вместе с текстом вопроса, разрушая кэш контекста Ollama
                # для ВСЕГО диалога на каждый запрос (система — самое начало
                # последовательности токенов). Хвост и так каждый раз новый,
                # поэтому добавление сюда ничего не портит. Работаем с копией
                # словаря, чтобы не мутировать реально сохранённую историю чата.
                digest = solutions_auto_digest(user_text)
                if digest and messages and messages[-1].get("role") == "user":
                    tail = dict(messages[-1])
                    tail["content"] = f"{digest}\n\n---\n\n{tail.get('content', '')}"
            ...
            for _ in range(MAX_TOOL_ITERATIONS):
                ...
                token_count = 0
                try:
                    for chunk in stream_ollama(messages, cancel_flag=cancel_flag, fast_path=fast_path, user_text=user_text):
...
def api_state():
    return jsonify({
        ...
        "preferred_sites_knowledge": state.preferred_sites_knowledge,
        "preferred_sites_shopping": state.preferred_sites_shopping,
        "preferred_sites_photo": state.preferred_sites_photo,
        "preferred_sites_video": state.preferred_sites_video,
        "reasoning_levels": [
...
PREFERRED_SITES_CATEGORIES = {
    "knowledge": "preferred_sites_knowledge",
    "shopping": "preferred_sites_shopping",
    "photo": "preferred_sites_photo",
    "video": "preferred_sites_video",
}


@app.route("/api/tool-categories", methods=["GET", "POST"])
def api_tool_categories():
    """Список категорий инструментов (для панели 'Pro') и ручное
    включение/выключение — выключенная категория исключается из схемы
    инструментов целиком (см. get_tools_schema). Также хранит режим
    "auto"/"manual" панели."""
    if request.method == "GET":
        categories = [
            {"key": key, "label": label, "enabled": key not in state.disabled_tool_categories}
            for key, (label, _names) in TOOL_CATEGORIES.items()
        ]
        return jsonify({"categories": categories, "tools_mode": state.tools_mode})

    body = request.get_json(force=True)
    if "tools_mode" in body:
        if body["tools_mode"] not in ("auto", "manual"):
            return jsonify({"ok": False, "error": "tools_mode должен быть 'auto' или 'manual'"}), 400
        state.tools_mode = body["tools_mode"]
    if "disabled_tool_categories" in body:
        disabled = body.get("disabled_tool_categories")
        if not isinstance(disabled, list) or any(c not in TOOL_CATEGORIES for c in disabled):
            return jsonify({"ok": False, "error": "Некорректный список категорий"}), 400
        state.disabled_tool_categories = disabled
    state.save()
    return jsonify({"ok": True, "disabled_tool_categories": state.disabled_tool_categories,
                     "tools_mode": state.tools_mode})


@app.route("/api/preferred-sites/<category>", methods=["GET", "POST", "DELETE"])
def api_preferred_sites(category):
    attr = PREFERRED_SITES_CATEGORIES.get(category)
    if not attr:
        return jsonify({"ok": False, "error": "Неизвестная категория"}), 404
    if request.method == "GET":
        return jsonify({"sites": getattr(state, attr)})
    ...
    if request.method == "POST":
        ...
        if not site:
            return jsonify({"ok": False, "error": "Пустой домен"}), 400
        lst = getattr(state, attr)
        if site not in lst:
            lst.append(site)
            state.save()
        return jsonify({"ok": True, "sites": lst})
    ...
    if request.method == "DELETE":
        ...
        if site == "all":
            ...
            lst.remove(site)
        state.save()
        return jsonify({"ok": True, "sites": getattr(state, attr)})
...
Формат ответа: сначала перечисли шаги, потом выполняй каждый. Используй маркированный список для шагов."""
        ...
        def worker():
            ...
            try:
                for chunk in stream_ollama(messages, cancel_flag=cancel_flag, user_text=task_text):
                    if "__cancelled__" in chunk:
...
def _run_ai_task(task_id, task):
    ...
    try:
        ...
        for _ in range(MAX_TOOL_ITERATIONS):
            ...
            token_count = 0
            try:
                for chunk in stream_ollama(messages, user_text=task_text):
                    if "__cancelled__" in chunk:
                        break
                    if "__replace_history__" in chunk:
                        continue
